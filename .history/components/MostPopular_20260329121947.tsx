...
export default async function MostPopular() {
  ...
  const transformedPosts = posts.map((post: { $id: string; title: string; prompt: string; thumbnail: string; video: string }) => ({
    $id: post.$id,
    title: post.title,
    prompt: post.prompt,
 ...
 <SwiperItems posts={transformedPosts} className="shadow-[-6px_0_10px_rgba(0,0,0,0.15)] lg:pr-20 bg-amber-700 " reverseTrue={true} reverse="rtl" items={mostPopular}/>
